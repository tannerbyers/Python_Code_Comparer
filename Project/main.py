...
from ListMostUsedWords import ListMostUsedWords
from BasicValidation import BasicValidation
...
#Iterates over each row in New Reject File, assigning it an index    
        ...
        #This will seperate each iteration of a full claim rejection (for testing)
            ...
            normalValidatedRow = BasicValidation(firstValidationIteratedRow)
        ...
        #Iterates over each row in Current Reject File, assigning it an index
            ...
            #Checks Accuracy of Two Strings
                accuracyOfCurrentString = CompareStrings(str(normalValidatedRow), str(currentOldRow))
            
            # Used only for reporting
                #TopWords.update(ListMostUsedWords(str(normalValidatedRow), str(currentOldRow)))
            ...
            #Sets Object to the most correct value
                if accuracyOfCurrentString > ClosestRow["Accuracy"]:
                    ...
                    ClosestRow["RejectionChecked"] = currentRow
    ...
    print(sorted(TopWords.items(), key = lambda kv:(kv[1], kv[0])))  
